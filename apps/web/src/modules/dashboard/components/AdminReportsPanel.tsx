...
import { Clock, TrendingUp, AlertTriangle, Moon, RefreshCw } from 'lucide-react';
...
export function AdminReportsPanel() {
  ...
  const fetchReport = useCallback(async () => {
    ...
    try {
      if (mode === 'daily') {
        setDailyData(await adminApi.getDailyReport(dateKey));
      } else {
        setWeeklyData(await adminApi.getWeeklyReport(weekKey) as WeeklySummary[]);
      }
    } catch (err: any) {
  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-wrap gap-4 items-center">
        ...
        {mode === 'daily' ? (
          <input type="date" value={dateKey} onChange={(e) => setDateKey(e.target.value)} className="input-theme max-w-[180px]" />
        ) : (
          <input
            type="week"
            value={weekKey}
            onChange={(e) => { if (e.target.value) setWeekKey(e.target.value); }}
            className="input-theme max-w-[180px]"
          />
        )}

        <button type="button" onClick={fetchReport} className="btn-secondary ml-auto">
      ...
      <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className="floating-card p-6">
        {error && (
